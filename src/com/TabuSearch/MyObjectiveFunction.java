...
public class MyObjectiveFunction implements ObjectiveFunction {
	...
	private Cost getTotalCostSwapVariation(MySolution sol, MySwapMove move, Cost varInsertCost, Cost varDeleteCost) 
	{
		Cost varCost = new Cost(sol.getCost());
		Route insertRoute = sol.getRoute(move.getInsertRouteNr());
		Route deleteRoute = sol.getRoute(move.getDeleteRouteNr());
	...
	private Cost evaluateInsertRoute(Route route, Customer customer, int position) {
		...
		if(route.isEmpty()) {
			...
			if(position == route.getCustomersLength()){
				...
				// variation of the time windows violation
				varCost.twViol +=  - customerAfter.getTwViol() + twViolCustomer + twViolNextCustomer;variation = arriveNextCustomer + waitingTimeNextCustomer - customerAfter.getArriveTime() - customerAfter.getWaitingTime();
